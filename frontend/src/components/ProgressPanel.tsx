...
export function ProgressPanel({ status, modelProgress, progress, onCancel }: ProgressPanelProps) {
  ...
  return (
    <section className="progress-card" aria-labelledby="progress-heading" aria-busy="true">
      ...
      <div>
        <p className="eyebrow">Processing on this device</p>
        <h2 id="progress-heading">{PHASES[currentIndex]?.label ?? 'Working on your audio'}</h2>
        <p aria-live="polite">
          {downloading
      ...
      <ol className="phase-list">
        {PHASES.map((phase, index) => (
          <li key={phase.key} data-state={index < currentIndex ? 'complete' : index === currentIndex ? 'current' : 'pending'}>
      ...
      <button type="button" onClick={onCancel} className="secondary-button">
        <X aria-hidden="true" /> Cancel processing
      </button>
    </section>
  );
}
